...
class GeneralLedger extends owl.Component {
    ...
    async load_data() {
        ...
        try {
            ...
            let data = await self.orm.call('account.general.ledger', "view_report", [[this.wizard_id], action_title,])
            self.state.account_data = data[0]
            self.state.account_records = data[1]
    ...
    gotoJournalItem(ev) {
        console.log(parseInt(ev.target.attributes["data-id"].value, 10))
        return this.action.doAction({
            type: "ir.actions.act_window",
    ...
    async applyFilter(val, ev, is_delete = false) {
        ...
        console.log(val.target.attributes["data-value"].value)
        if (ev) {
            ...
            if (val.target.name === 'start_date') {
                ...
                if (!val.target.classList.contains("selected-filter")) {
                    console.log(val.target.attributes["data-id"].value)
                    let accountId = parseInt(val.target.attributes["data-id"].value, 10);
                    this.state.selected_account.push(accountId);
                    val.target.classList.add("selected-filter");
                } else {
                    const updatedList = this.state.selected_account.filter(item => item !== parseInt(val.target.attributes["data-id"].value, 10));
                    this.state.selected_account = updatedList
                    val.target.classList.remove("selected-filter");
                }
            } else if (val.target.attributes["data-value"].value == 'analytic') {
                if (!val.target.classList.contains("selected-filter")) {
        ...
        let filtered_data = await this.orm.call("account.general.ledger", "get_filter_values", [this.state.selected_account, this.state.selected_journal_list, this.state.date_range, this.state.options, this.state.selected_analytic_list, this.state.method]);
