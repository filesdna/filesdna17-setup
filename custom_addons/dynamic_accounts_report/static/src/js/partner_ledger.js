...
//            self.state.data = await self.orm.call("account.partner.ledger", "view_report", [[this.wizard_id], action_title,]);
            ...
            let data = await self.orm.call("account.partner.ledger", "view_report", [[this.wizard_id], action_title,]);
            self.state.data = data[0]
            self.state.partner_data = data[1]
    ...
    async applyFilter(val, ev, is_delete = false) {
        ...
        if (ev) {
            ...
            } else if (val.target.querySelector('span') && val.target.querySelector('span').textContent) {
                this.state.selected_partner = val.target.querySelector('span').textContent.split(',').map(Number);
