...
def _get_product_ribbon(product):
    ribbon_class = product.website_ribbon_id.html_class or ''
    position = ''
    if 'left' in ribbon_class:
        position = 'left'
    elif 'right' in ribbon_class:
        position = 'right'
    bgColor = {'text-bg-danger':'#DC4C64','text-bg-success':'#14A44D','text-bg-warning':'#E4A11B','text-bg-primary':'#54B4D3'}
    bg_color = ''
    textColor = ''
    if 'text-bg-danger' in ribbon_class:
        bg_color = bgColor['text-bg-danger']
        textColor = '#FFFFFF'
    elif 'text-bg-success' in ribbon_class:
        bg_color = bgColor['text-bg-success']
        textColor = '#FFFFFF'
    elif 'text-bg-warning' in ribbon_class:
        bg_color = bgColor['text-bg-warning']
        textColor = '#120f07'
    elif 'text-bg-primary' in ribbon_class:
        bg_color = bgColor['text-bg-primary']
        textColor = '#FFFFFF'
    return {
        'ribbon_message': product.website_ribbon_id.html or '',
        'position': position,
        'text_color': textColor if textColor else product.website_ribbon_id.text_color or '#589ff5',
        'bg_color': bg_color if bg_color else product.website_ribbon_id.bg_color or '#f7f9fa'
    }
...
def _getProductData(p_data, context={}):
    ...
    result = []
    for prod in p_data:
        comb_info = prod.with_context(context)._get_combination_info(combination=False, product_id=False,add_qty=1,parent_combination=False, only_template=False)
        result.append({
            ...
            'ribbon': _get_product_ribbon(prod)
        })
    return result
...
def _get_product_domain():
    return [("sale_ok", "=", True), ("is_mobikul_available", "=", True),('is_published','=',True)]
