...
class DmsDirectory(models.Model):
    ...
    @api.depends("complete_group_ids", "complete_group_ids.users")
    ...
    def action_restore_multiple_directories(self):
        for rec in self:
            rec.action_restore_directory()

    @api.model
    def action_delete(self):
        for rec in self:
            rec.unlink_directory()

    def action_restore_directory(self):
    ...
    def unlink_directory(self):
        ...
        if self.id != False:
            if self.is_deleted == True:
                ...
                groups = self.complete_group_ids.ids
