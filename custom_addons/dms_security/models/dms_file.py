...
class DMSFileSecurity(models.Model):
    ...
    dms_security_id = fields.Many2one(comodel_name='dms.security', string='Security Option',
                                      compute='onchange_security_user_id', store=True)
    ...
    directory_is_unlocked = fields.Boolean(
        string="Directory Unlocked",
        related='directory_id.is_unlocked',
        store=True
    )

    def action_open_file(self):
        """ Prevent file access if the parent directory is locked """
        for file in self:
            if file.active_security and not file.directory_is_unlocked:
                raise UserError(_("Access Denied: Unlock the directory first to access this file."))
    ...
    def action_open_kanban_authenticator(self):
        if self.active_security:
            if self.env.user.id == self.dms_security_id.user_id.id:
                ...
                message_id = self.env['message.wizard'].create(
                    {'message': _("This file is private you are not allow to open it")})
                return {
