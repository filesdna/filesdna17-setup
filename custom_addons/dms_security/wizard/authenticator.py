...
class TOTPDMAWizard(models.TransientModel):
    ...
    def enable(self):
        ...
        if self._totp_try_setting_dms(self.secret, c):
            self.secret = '' # empty it, because why keep it until GC?
            google_2fa = self.env['dms.security'].search([('user_id','=',self.env.user.id),('selection','=','g2fa')])
            google_2fa.is_required = True
            return {
    ...
    def _totp_try_setting_dms(self, secret, code):
        secret = compress(secret).upper()
        match = TOTP(base64.b32decode(secret)).match(code)
        if match is None:
            _logger.info("2FA enable: REJECT CODE for %s %r", self, self.env.user.login)
            return False
        self.env.user.sudo().totp_secret_dms = secret
