...
class DataSetInherit(DataSet):
    ...
    def do_search_read(self, model, fields=False, offset=0, limit=False, domain=None, sort=None):
        """
        @inherited
            For Hide Admin user from list of users for tenant user
        """
        ...
        if Model._name == 'res.users' and request._uid != 2:
            domain += [('id', '!=', 2)]

        res = super(DataSetInherit, self).do_search_read(model, fields, offset=offset, domain=domain, sort=sort)
        return res
...
class Home(Home):
    ...
    @http.route(['/check_login_user'], type='json', auth="public", website=True)
    def login_user_info(self, **post):
        ret = False
        if request._uid == 2:
            ret = True
