...
class res_users(models.Model):
    ...
    def create(self, val_list):
        ...
        for vals in val_list:
            if self._uid not in [1, 2, 3, 4, 5]:
                ...
                # Allowed to create multi companes for tenant users
            ...
            try:
                res = super(res_users, self).create(val_list)
    ...
    def write(self, vals):
        ...
        if self._uid not in [1, 2, 3, 4, 5]:
            ...
            # Allowed to give access for multi comapany for self which is tenant user
            # if group_id3 and 'in_group_' + str(group_id3.id) in vals and vals['in_group_' + str(group_id3.id)] is True:
            #     groups = groups + " 'Multi Companies'"
...
class res_group(models.Model):
    ...
    def create(self, vals):
        ...
        res_grp = self.env['res.groups'].browse(int(res.id))
        if res:
            ...
            for user in res_users_obj.search([]):
                ...
                if user.has_group("openerp_saas_tenant_extension.group_psuedo_admin") and not res_grp.name == "Tax display B2C":
...
class IrUiMenu(models.Model):
    ...
    #         # res_groups_obj = self.env['res.groups']
    #         # group_id = res_groups_obj.search([('name', '=', 'Tenant Super User')])
    #         # if group_id:
    #         #     self._cr.execute(
    #         #         "select * from ir_ui_menu_group_rel where menu_id=%s and gid=%s" % (res.id, group_id.id))
    #         #     exist = self._cr.fetchall()
