...
server {
    ...
    location / {
        proxy_pass  http://xyz_domain_name;
...
server {
    ...
    ssl_certificate_key ssl1;
    ssl_certificate ssl2;
}
...
class tenant_database_list(models.Model):
    ...
    def action_remove_client_domain(self):
        ...
        _logger.info("action remove client domia")
        ICPSudo = self.env['ir.config_parameter'].sudo()
    ...
    def action_set_client_domain(self):
        for domain in self.domain_masking_fields:
            ...
            if not domain.client_domain or not domain.client_ssl1 or not domain.client_ssl2:
...
class domain_masking(models.Model):
    ...
    def action_remove_client_domain(self):
        ...
        if self.domain_type == 'https':
            ...
            if web_conf == 'apache':
                ...
                name = name.replace(".", "")
                name = name.replace("/", "")
                file_string = file_string.replace('xyz_main_name', 'https://' + self.client_domain + '.com')
            ...
            if self.tenant_db_management.name and apache_ssl_path:
                if self.client_ssl1_filename:
                    file_string = file_string.replace('ssl1',
                                                  apache_ssl_path + "/" + self.tenant_db_management.name + "/" + self.client_ssl1_filename)
                if self.client_ssl2_filename:
                    file_string = file_string.replace('ssl2',
                                                  apache_ssl_path + "/" + self.tenant_db_management.name + "/" + self.client_ssl2_filename)
                if self.client_ssl3_filename:
                    file_string = file_string.replace('ssl3',
                                                  apache_ssl_path + "/" + self.tenant_db_management.name + "/" + self.client_ssl3_filename)
    ...
    def action_set_client_domain(self):
        ...
        if self.domain_type == 'https':
            if not self.client_domain or not self.client_ssl1 or not self.client_ssl2:
                raise ValidationError(_("Please fill all Fields!"))
        else:
        ...
        if self.domain_type == 'https':
            ...
            if web_conf == 'apache':
                ...
                name = name.replace(".", "")
                name = name.replace("/", "")
                file_string = file_string.replace('xyz_main_name', str(self.client_domain) + ".com")
            file_string = file_string.replace('xyz_domain_name', str(self.client_domain))
            file_string = file_string.replace('ssl1',
                                              apache_ssl_path + "/" + self.tenant_db_management.name + "/" + self.client_ssl1_filename)
            file_string = file_string.replace('ssl2',
                                              apache_ssl_path + "/" + self.tenant_db_management.name + "/" + self.client_ssl2_filename)
