...
//		    });
               ...
               $('.input-div-db').keypress(function(key) {
              if (this.selectionStart === 0 && key.charCode === 95) {
                return false;
              }
            });
			...
			console.log("payment valuess>>>>",payment_value)
			// alert("ssss");
				...
				else
				{
				   $.ajax({
						type: "POST",
						dataType: "json",
						url: "/check/payment/method",
						contentType: "application/json; charset=utf-8",
						data: JSON.stringify({ 'jsonrpc': "2.0", 'method': "call", 'params': { 'payment_value': payment_value} }),
         ...
         var webUrl = window.location.pathname
			...
			if(webUrl.includes('/shop/payment'))
                    {
					console.log("insided calling shop payment >>>")
                    $.ajax({
							...
							console.log("payment terms>>>>>>>>>", $("#payement_base").val(result['select_payment_option']))
                            console.log('result>>>>>>', result)
                            if (result['select_payment_option'] == 'False'){
                                // ajax.post('/shop/payment', result)
                                console.log("YYYYYYYYYYYYYYY", $('#payment_method_trial').find(':input[type="submit"]'))
                                // $('#payment_method_trial').find(':input[type="submit"]').removeAttr('disabled')
                                $('input:radio[name="o_payment_radio"]').attr('checked', true);
                            }
                            }
                            }
                            });
...
//                        });
			...
			if ($('input[type=radio]')) {
				...
				$('#new').on('click', function() {
				    ...
				    $('.package').show();
                    $('.product_check').prop('checked', false)
                    document.getElementById('m_num_apps').innerHTML = _t('0 Apps');
                    document.getElementById('num_apps').innerHTML = _t('0 Apps');
...
//					}
		...
		$(function() {
			$('#pay').on('click',function() {
                console.log("second proceed to pay click")
			    var ret = true
				var dbusers = document.getElementById('db_option')
				var nu = ''
				if (dbusers != null)
...
//                console.log('____________', document.getElementById('languages'));
...
//	    console.log('______________________________________',window.location.pathname)
