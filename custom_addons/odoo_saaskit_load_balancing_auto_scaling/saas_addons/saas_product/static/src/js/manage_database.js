...
//	user decrease
	$('#save_user_decrease').on('click', function() {
            ...
            var current_users = document.getElementById('decrease_user2');
            var plan_users = getPlanUsers();
            var user_diff = parseFloat(current_users.innerHTML) - parseFloat(user_count_decrease);
            console.log(user_diff,' user_diff');
            if(user_diff < plan_users){
                 Dialog.alert(self, _t("You can't decrease no of users below ")+ plan_users, {
                        title: _t('Error'),
                    });
                    return false
            }
            ...
            if(parseInt(user_count_decrease) == 0){
				Dialog.alert(self, _t("Value should be more than zero"), {
                        title: _t('Error'),
                    });
				return false
			}
		...
		$('#save_user').on('click', function() {
			...
			$.ajax({
				...
				success : function(data) {
				    console.log("============atatatatatatat======",data)
                    data = parseInt(data)
