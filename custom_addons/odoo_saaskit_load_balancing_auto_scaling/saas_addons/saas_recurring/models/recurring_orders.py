...
class agreement(models.Model):
    ...
    invoice_term_id = fields.Many2one('recurring.term', 'Invoicing Term', store=True, readonly=True,
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            res = super(agreement, self).create(vals_list)
    ...
    def change_invoicing_term(self):
        wizard_obj = self.env['change.invoicing.term.wizard'].sudo()
        res_id = wizard_obj.create({'invoice_term_id': self.invoice_term_id.id})
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'change.invoicing.term.wizard',
