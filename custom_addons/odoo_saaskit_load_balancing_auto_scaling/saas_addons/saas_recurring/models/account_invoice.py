...
class AccountPayment(models.Model):
    ...
    def action_post(self):
        ...
        # print('Action Post Method: ', self.move_id, self._context)
        if 'active_id' in self._context or self:
            if 'active_id' not in self._context:
                move = self.env['account.move'].search([('name', '=', self.ref.split('-')[0])])
                # move = self.move_id
            else:
            ...
            if move:
                ...
                # print("\n\n\n________so_obj move : ",move,self.reconciled_bill_ids,self.reconciled_invoice_ids,self.website_id,self.invoice_origin, move, so_obj, so_obj.saas_order)
                # sadfasd
                if so_obj.saas_order:
                    db_name = so_obj.instance_name
                    tenant_db = self.env['tenant.database.list'].search([('name', '=', db_name)])
                    # if move.invoice_type == 'rent':
                    ## XMLRPC CONNECTION
                    ...
                    if allow_to_active_db:
                        ...
                        if not new_exp_date:
                            ...
                            # print('New exp Date', new_exp_date, current_exp_date)
                            if not expiry_date_changed:
                                while True:
                                    ...
                                    # print('New exp Date', new_exp_date, 'today', today)
                    ...
                    mail_template_id = self.env['ir.model.data'].check_object_reference('saas_sale', 'email_template_renew_subscription')
