    ...
    def tenant_db_connection(self, active_id):
        ...
        if server:
            try:
                ...
                data = False
                _logger.exception('Exception: %s', e)
                raise UserError(_('Container Connection Refused '))
        else:
...
class TenantDBList(models.Model):
    ...
    isdeactivated = fields.Boolean(related="stage_id.is_deactivated", )
    isterminated = fields.Boolean(related="stage_id.is_purge", )
