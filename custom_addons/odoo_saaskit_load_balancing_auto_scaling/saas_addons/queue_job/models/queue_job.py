...
class QueueJob(models.Model):
    ...
    # Filed for Tenant for identify jobs
    tenant_name = fields.Char(
        compute="_compute_tenant_job", index=True
    )

    db_name = fields.Char("Database Name")
    is_deleted = fields.Boolean('Deleted')
    
    @api.depends("records","tenant_name")
    def _compute_tenant_job(self):
        self.clear_caches()
        self.env['sale.order'].clear_caches()
        for record in self:
            model = repr(record.records)
            # record.tenant_name = record.records.instance_name
            _logger.info('---------------- {} '.format(model))
            try:
                if record.method_name == 'post_database_update':
                    if record.records and record.records.instance_name:
                        record.tenant_name = '{}{}'.format(record.records and record.records.instance_name and record.records.instance_name.title(), ': Job - 3')
                        record.db_name = record.records and record.records.instance_name or ''
                    else:
                        record.tenant_name = ''
                        record.db_name = ''
                elif record.method_name == 'create_database':
                    if record.records and record.records.instance_name:
                        record.tenant_name = '{}{}'.format(record.records and record.records.instance_name and record.records.instance_name.title(), ': Job - 1')
                        record.db_name = record.records and record.records.instance_name or ''
                    else:
                        record.tenant_name = ''
                        record.db_name = ''
                elif record.method_name == 'post_installation_work':
                    if record.records and record.records.instance_name:
                        record.tenant_name = '{}{}'.format(record.records and record.records.instance_name and record.records.instance_name.title(), ': Job - 2')
                        record.db_name = record.records and record.records.instance_name or ''
                    else:
                        record.tenant_name = ''
                        record.db_name = ''
                elif record.method_name == 'delete_database':
                    record.tenant_name = '{} : Delete Database'.format(record.records.tenant_id.name.title())
                    record.db_name = record.records.tenant_id.name
    ...
    def requeue(self):
        job_ids = False
        for rec in self:
            job_ids = self.search([('db_name','=',rec.db_name)],order="id asc")
        if job_ids:
            for job in job_ids:
                job._change_job_state(PENDING)
        return True
