...
class AccountMoveLineInherit(models.Model):
    ...
    def _get_price_total_and_subtotal_model(self, price_unit, quantity, discount, currency, product, partner, taxes,
                                            move_type):
        print('============================================= Start Compute =================================================')
        print('\n\nCompute Invoice lines vals :',self.move_id.billing, price_unit, quantity, discount, currency, product, partner, taxes,
              self, move_type,self.move_id.invoice_origin)
        ...
        # print("months \n", months)
        # quantity =
        line_discount_price_unit = price_unit * (1 - (discount / 100.0)) * months

        subtotal = quantity * line_discount_price_unit

        print("all vals>>>>>>>>>>>>>>>>>>>>>>",price_unit,quantity, line_discount_price_unit, no_of_users, months, discount)
        ...
        if taxes:
            ...
            taxes_res = taxes._origin.with_context(force_sign=force_sign).compute_all(line_discount_price_unit,
                                                                                      ...
                                                                                      users=no_of_users, months=months)
            print('________taxes : ', taxes_res)
