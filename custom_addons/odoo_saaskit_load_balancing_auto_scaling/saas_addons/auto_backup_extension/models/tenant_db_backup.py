...
class TenantBackup(models.Model):
    ...
    def psycopg2_connection(self):
        rec = self.search([])  # Get Saasmaster Db name
        if rec.postgres_pwd and rec.postgres_user and rec.host:
            try:
                conn = psycopg2.connect(database='saasmaster_v17',
    ...
    def check_owner_of_db(self, conn, db_name):
        try:
            with closing(conn.cursor()) as cur:
                ...
                cmd = "SELECT d.datname as saasmaster_v17,pg_catalog.pg_get_userbyid(d.datdba) as Owner FROM pg_catalog.pg_database d;"
            ...
            owndbs = []
            owner = None
            # db = self.env['db.backup'].search([])  # Get Saasmaster Db name
            for row in rows:  # Getting Saasmaster owner
                if row[0] == db_name:
                    owner = row[1]

            for row in rows:  # Get all dbs of Saasmaster db owner
                if owner == row[1]:
                    owndbs.append(row[0])
    ...
    def schedule_backup(self):
        print("===========schedule_backup======schedule_backup========",self)
        conn = self.sudo().psycopg2_connection()  # Get Connection object of Postgres
        print("=======================conn==============",conn)
        if conn:
            conf_ids = self.sudo().search([])
            for rec in conf_ids:
                try:
                ...
                try:
                    saas_owner = self.sudo().check_owner_of_db(conn, rec.name)  # get owner of saasmaster_v16
                    _logger.info('\n\n\nStarted to Dump All Databases ...............................\n\n\n')
                    for db in saas_owner['owndbs']:
                        self.sudo().zip_convert_database(db, rec)
    ...
    def zip_convert_database(self, tenanat_db, rec):
        ...
        if rec.sftp_write is True:
            try:
                ...
                for file in sftp.listdir(path_to_write_to):
                    tenantPathName = os.path.basename(os.path.normpath(file))
                    if tenanat_db == tenantPathName:
        ...
        if rec.autoremove:
            ...
            for f in os.listdir(directory):
                ...
                tenantPathName = os.path.basename(os.path.normpath(fullpath))
                if tenanat_db == tenantPathName:
