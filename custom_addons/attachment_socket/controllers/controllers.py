    ...
    def get_attachment(self, attachment_id):
        ...
        if attachment.active_security:
            if attachment.security_user_id.id != uid :
                return Response(f"Unathourized, this file is locked by user {attachment.security_user_id.name}", status=403)
            ...
            if uid in group.users.ids:
                for dir in group.directory_ids:
                    ...
                    if dir.is_root_directory and not dir.is_template:
                        if dir.active_security:
                            if dir.security_user_id.id == uid:
                                data = {
...
# //////////////////////////// check file name //////////////////////////////////////////////////////////////////////////
    ...
    @http.route('/api/attachments/file-check/<string:file_name>', auth='user', type='json', methods=['POST'], csrf=False)
    def check_file(self,file_name=None,**kw):
        ...
        domain = [('name', '=', file_name),('is_deleted','=',False),('directory_id.id','in',dir_result),('company_id','=',company_id)]
        file_model = request.env['dms.file'].sudo()
        file = file_model.search(domain)
        if file:
...
# ///////////////////////////////// sub directories //////////////////////////////////////
    @http.route('/api/attachments/sub-directories/<int:parent_dir>', auth='user', type='json', methods=['GET'], csrf=False)
    def sub_dir_list(self,parent_dir=None):
        result = []
        uid = request.env.user.id
        sub_dirs = request.env['dms.directory'].search([('is_root_directory','=',False),('parent_id','=',parent_dir)])
...
# //////////////////////////////////////// Search Result ///////////////////////////////////////////////////
    ...
    @http.route('/api/attachments/search-result/<int:company_id>', auth='user', type='json', methods=['POST'], csrf=False)
