...
echo "Updating PostgreSQL configuration for Odoo optimization..."
...
sed -i "/^#\?work_mem/c\work_mem = 50MB" $PG_CONF
sed -i "/^#\?maintenance_work_mem/c\maintenance_work_mem = 1GB" $PG_CONF
...
sed -i "/^#\?wal_level/c\wal_level = replica" $PG_CONF
sed -i "/^#\?max_wal_senders/c\max_wal_senders = 10" $PG_CONF
